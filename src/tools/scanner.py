...
async def run_backtest(ticker: str,
                       period:         str   = "1y",
                       tick_threshold: float = DEFAULT_TICK_THRESH,
                       vol_threshold:  float = DEFAULT_VOL_THRESH) -> dict:
    """Backtest MA Ketat signal on a ticker's historical data.

    Finds all past MA Ketat signals and measures 7-day forward returns.

    Args:
        ticker:         IDX ticker symbol
        period:         Historical lookback ("1y" or "2y", default "1y")
        tick_threshold: MA range_ticks threshold (default 6.0)
        vol_threshold:  Volatility threshold in % (default 3.8)
    """
    try:
        normalized = validate_ticker(ticker)
    except ValueError as e:
    ...
    period = period if period in ("1y", "2y") else "1y"
    tick_threshold = float(tick_threshold or DEFAULT_TICK_THRESH)
    vol_threshold  = float(vol_threshold  or DEFAULT_VOL_THRESH)

    params = {"period": period, "tt": tick_threshold, "vt": vol_threshold}
    cached = cache.get("ma_backtest", normalized, params)
    if cached is not None:
        return cached

    try:
        result = await asyncio.wait_for(
            asyncio.to_thread(_backtest_single, normalized, period, tick_threshold, vol_threshold),
            timeout=_T_BACKTEST,
        )
    except asyncio.TimeoutError:
        return {"error": True, "error_type": "timeout",
                "message": f"Backtest for {normalized} timed out.",
                "partial_data": None, "suggestion": "Try again."}
    except Exception as e:
        return {"error": True, "error_type": "data_unavailable",
    ...
    if not result.get("error"):
        cache.set("ma_backtest", normalized, result, _TTL_BACKTEST, params)
    return result


def _backtest_single(normalized: str, period: str,
                     tick_thresh: float, vol_thresh: float) -> dict:
    data = _download_batch([_to_jk(normalized)], period=period)
    df   = data.get(normalized)
    # Require 140 rows: 125 for BB squeeze warm-up + 7 horizon + buffer (C-6)
    if df is None or df.empty or len(df) < 140:
        return {"error": True, "error_type": "data_unavailable",
                "message": f"Insufficient data for backtest of {normalized} (need ≥140 trading days).",
                "partial_data": None, "suggestion": "Try a longer period or different ticker."}
    ...
    horizon = 7
    # Start at 130: ensures BB squeeze (125-row rolling min) is fully warmed up (C-6)
    for i in range(130, len(enriched) - horizon):
        row    = enriched.iloc[i]
        future = enriched.iloc[i + horizon]
        if not _passes_entry_filters(row, tick_thresh, vol_thresh):
            continue

        entry_price  = _f(row.get("Close"))
        exit_price   = _f(future.get("Close"))
        if entry_price is None or exit_price is None:
            continue

        ret_pct = ((exit_price - entry_price) / entry_price) * 100
        returns_7d.append(ret_pct)
        try:
            date_str = str(enriched.index[i].date())   # L-3: guard non-DatetimeIndex
        except AttributeError:
            date_str = str(enriched.index[i])
        signals_found.append({
            "date":          date_str,
            "close":         entry_price,
            "return_7d_pct": safe_round(ret_pct, 2),
            "win":           ret_pct > 0,
        })
    ...
    if n == 0:
        return {
            ...
            "note":           "No historical MA Ketat signals found in this period.",
            "disclaimer":     _DISCLAIMER,
        }

    wins = sum(1 for r in returns_7d if r > 0)
    arr  = np.array(returns_7d)
    max_dd = float(arr.min()) if len(arr) else None

    return {
        "ticker":           normalized,
        "period":           period,
        "n_signals":        n,
        "win_rate_pct":     safe_round((wins / n) * 100, 1),
        "avg_return_pct":   safe_round(float(arr.mean()), 2),
        "median_return_pct": safe_round(float(np.median(arr)), 2),
        "max_drawdown_pct": safe_round(max_dd, 2),
        "avg_hold_days":    horizon,
        "best_return_pct":  safe_round(float(arr.max()), 2),
        "worst_return_pct": safe_round(float(arr.min()), 2),
        "signals":          signals_found[-20:],   # last 20 signals
        "parameters_used": {
            "tick_threshold": tick_thresh,
            "vol_threshold":  vol_thresh,
        },
