...
_TIMEOUT = 30.0  # 3 sources × ~8s each max
...
async def get_stock_news(ticker: str, limit: int = 10) -> dict:
    ...
    try:
        data = await asyncio.wait_for(
            scrape_news(normalized, limit=limit),
            timeout=_TIMEOUT,
        )
        ...
        return {
            "error": True,
            "error_type": "timeout",
            "message": f"News fetch for {normalized} timed out after {_TIMEOUT:.0f}s.",
            "partial_data": {"ticker": normalized},
            "suggestion": "Try again later. News sources may be slow.",
        }
        ...
        return {
            ...
            "message": f"Failed to fetch news for {normalized}: {e}",
            "partial_data": {"ticker": normalized},
